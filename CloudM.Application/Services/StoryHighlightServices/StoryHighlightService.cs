...
namespace CloudM.Application.Services.StoryHighlightServices
{
    public class StoryHighlightService : IStoryHighlightService
    {
        ...
        public async Task<StoryHighlightGroupMutationResponse> CreateGroupAsync(Guid currentId, StoryHighlightCreateGroupRequest request)
        {
            ...
            {
                throw new BadRequestException("At least one story must be selected.");
            }
            ...
            var selectedStories = await _storyHighlightRepository.GetArchiveStoriesByIdsForOwnerAsync(currentId, normalizedStoryIds);
            if (selectedStories.Count != normalizedStoryIds.Count)
            {
                throw new BadRequestException("Some selected stories are invalid or no longer available.");
            }

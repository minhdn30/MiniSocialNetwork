...
namespace CloudM.Tests.Services
{
    public class CommentReactServiceTests
    {
        ...
        public async Task ToggleReactOnComment_NotReacted_AddsReactAndReturnsResponse()
        {
            ...
            _notificationServiceMock.Setup(x => x.EnqueueAggregateEventAsync(It.IsAny<NotificationAggregateEvent>(), It.IsAny<CancellationToken>()))
            ...
            _notificationServiceMock.Verify(x => x.EnqueueAggregateEventAsync(
                It.Is<NotificationAggregateEvent>(eventModel =>
                    eventModel.RecipientId == comment.AccountId &&
                    eventModel.Action == NotificationAggregateActionEnum.Upsert &&
                    eventModel.Type == NotificationTypeEnum.CommentReact &&
                    eventModel.AggregateKey == NotificationAggregateKeys.CommentReact(commentId) &&
                    eventModel.SourceType == NotificationSourceTypeEnum.CommentReact &&
                    eventModel.SourceId == accountId &&
                    eventModel.ActorId == accountId &&
                    eventModel.TargetKind == NotificationTargetKindEnum.Post &&
                    eventModel.TargetId == postId &&
                    eventModel.KeepWhenEmpty == false),
        ...
        public async Task ToggleReactOnComment_AlreadyReacted_RemovesReactAndReturnsResponse()
        {
            ...
            _notificationServiceMock.Verify(x => x.EnqueueAggregateEventAsync(
                It.Is<NotificationAggregateEvent>(eventModel =>
                    eventModel.RecipientId == comment.AccountId &&
                    eventModel.Action == NotificationAggregateActionEnum.Deactivate &&
                    eventModel.Type == NotificationTypeEnum.CommentReact &&
                    eventModel.AggregateKey == NotificationAggregateKeys.CommentReact(commentId) &&
                    eventModel.SourceType == NotificationSourceTypeEnum.CommentReact &&
                    eventModel.SourceId == accountId &&
        ...
        [Fact]
        public async Task ToggleReactOnComment_ReplyReact_ShouldEnqueueReplyReactNotification()
        {
            // Arrange
            var commentId = Guid.NewGuid();
            ...
            _notificationServiceMock.Verify(x => x.EnqueueAggregateEventAsync(
                It.Is<NotificationAggregateEvent>(eventModel =>
                    eventModel.RecipientId == replyOwnerId &&
                    eventModel.Action == NotificationAggregateActionEnum.Upsert &&
                    eventModel.Type == NotificationTypeEnum.ReplyReact &&
                    eventModel.AggregateKey == NotificationAggregateKeys.ReplyReact(commentId) &&
                    eventModel.SourceType == NotificationSourceTypeEnum.ReplyReact &&
                    eventModel.SourceId == accountId &&
                    eventModel.ActorId == accountId &&
                    eventModel.TargetKind == NotificationTargetKindEnum.Post &&
                    eventModel.TargetId == postId),
        ...
        [Fact]
        public async Task ToggleReactOnComment_SelfReact_ShouldNotEnqueueNotification()
        {
            // Arrange
            var commentId = Guid.NewGuid();
            var postId = Guid.NewGuid();
            var accountId = Guid.NewGuid();
            var comment = TestDataFactory.CreateComment(commentId: commentId, postId: postId, accountId: accountId);
            var post = TestDataFactory.CreatePost(postId: postId, privacy: PostPrivacyEnum.Public);
            ...
            // Assert
            result.IsReactedByCurrentUser.Should().BeTrue();
            _notificationServiceMock.Verify(x => x.EnqueueAggregateEventAsync(It.IsAny<NotificationAggregateEvent>(), It.IsAny<CancellationToken>()), Times.Never);
        }
